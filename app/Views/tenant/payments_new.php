...
<body>
            ...
            <div class="col-md-8">
                <div class="content-section">
                    <div class="section-header">
                        ...
                        <div>
                            <span class="badge-success me-2"><i class="bi bi-check-circle"></i> <?= count(array_filter($payments ?? [], fn($p) => $p['status'] === 'approved')) ?> Approved</span>
                            <span class="badge-warning me-2"><i class="bi bi-clock"></i> <?= count(array_filter($payments ?? [], fn($p) => $p['status'] === 'pending')) ?> Pending</span>
                            <span class="badge-danger"><i class="bi bi-x-circle"></i> <?= count(array_filter($payments ?? [], fn($p) => $p['status'] === 'rejected')) ?> Rejected</span>
                        </div>
                    </div>
                                ...
                                <?php if (!empty($payments)): ?>
                                    <?php foreach ($payments as $payment): ?>
                                        <tr>
                                            ...
                                            <td>
                                                <button class="action-btn btn-view" onclick="viewPayment('<?= date('M d, Y H:i', strtotime($payment['created_at'])) ?>', '<?= number_format($payment['amount'], 2) ?>', '<?= esc($payment['payment_method']) ?>', '<?= esc($payment['reference_number']) ?>', '<?= esc($payment['notes']) ?>', '<?= $payment['status'] ?>')">
    ...
    <script>
        function viewPayment(date, amount, method, reference, notes, status) {
            ...
            let statusHtml = '';
            if (status === 'approved') {
                statusHtml = '<span class="badge-success"><i class="bi bi-check-circle"></i> Approved</span>';
            } else if (status === 'pending') {
                statusHtml = '<span class="badge-warning"><i class="bi bi-clock"></i> Pending</span>';
            } else {
                statusHtml = '<span class="badge-danger"><i class="bi bi-x-circle"></i> Rejected</span>';
            }
